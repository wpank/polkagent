...
async fn mi_01_semantic_cross_agent_isolation() {
    ...
    let results = svc
        .recall(bob, "NPoS consensus", 10)
        .await
        .expect("bob recall");
    assert!(
        results.is_empty(),
        "MI-01: bob must not see alice's semantic memory"
    );
}
...
async fn mi_02_episodic_cross_agent_isolation() {
    ...
    assert!(
        results.is_empty(),
        "MI-02: bob must not see alice's episodic memory"
    );
}
...
async fn mi_03_procedural_cross_agent_isolation() {
    ...
    svc.remember(
        alice,
        "Always use transfer_keep_alive",
    ...
    let results = svc
        .recall(bob, "transfer_keep_alive", 10)
        .await
        .expect("bob recall");
    assert!(
        results.is_empty(),
        "MI-03: bob must not see alice's procedural memory"
    );
}
...
async fn mi_04_reverse_agent_isolation() {
    ...
    let results = svc
        .recall(alice, "canary network", 10)
        .await
        .expect("alice recall");
...
async fn mi_05_episode_cross_agent_isolation() {
    ...
    let ep_id = svc
        .start_episode(alice, "Alice's staking session")
        .await
        .expect("start episode");
    svc.remember(
        alice,
        "Episode memory about XCM",
        MemoryType::Episodic,
        None,
    )
    .await
    ...
    assert!(
        results.is_empty(),
        "MI-05: bob must not see alice's episode memory"
    );
    let _ = ep_id;
}
...
async fn mi_06_same_episode_title_isolated() {
    ...
    let _ep_a = svc
        .start_episode(alice, "Governance review")
        .await
        .expect("ep alice");
    let _ep_b = svc
        .start_episode(bob, "Governance review")
        .await
        .expect("ep bob");

    svc.remember(
        alice,
        "Alice voted on referendum 42",
        MemoryType::Episodic,
        None,
    )
    .await
    ...
    let results = svc
        .recall(bob, "referendum 42", 10)
        .await
        .expect("bob recall");
    assert!(
        results.is_empty(),
        "MI-06: same episode title must not leak"
    );
}
...
async fn mi_08_forget_does_not_cross_agents() {
    ...
    svc.remember(
        alice,
        "Parachain auctions happen quarterly",
        MemoryType::Semantic,
        None,
    )
    .await
    ...
    let alice_results = svc
        .recall(alice, "parachain auctions", 10)
        .await
        .expect("alice recall");
    let bob_results = svc
        .recall(bob, "parachain auctions", 10)
        .await
        .expect("bob recall");
...
async fn mi_09_public_memory_isolated_by_agent() {
    ...
    svc.remember(
        alice,
        "Public fact about Polkadot relay chain",
        MemoryType::Semantic,
        None,
    )
    .await
    ...
    assert!(
        results.is_empty(),
        "MI-09: public memory is still agent-scoped"
    );
}
...
async fn mi_10_confidential_memory_isolated_by_agent() {
    ...
    svc.remember(
        alice,
        "Alice's private key derivation path",
        MemoryType::Semantic,
        None,
    )
    .await
    ...
    let results = svc
        .recall(bob, "private key derivation", 10)
        .await
        .expect("recall");
    assert!(
        results.is_empty(),
        "MI-10: confidential memory isolated by agent"
    );
}
...
async fn mi_11_restricted_memory_isolated_by_agent() {
    ...
    svc.remember(
        alice,
        "Validator key material for node-001",
        MemoryType::Procedural,
        None,
    )
    .await
    ...
    let results = svc
        .recall(bob, "validator key material", 10)
        .await
        .expect("recall");
    assert!(
        results.is_empty(),
        "MI-11: restricted memory isolated by agent"
    );
}
...
async fn mi_12_internal_memory_isolated_by_agent() {
    ...
    assert!(
        results.is_empty(),
        "MI-12: internal memory isolated by agent"
    );
}
...
async fn mi_13_recent_memory_not_visible_to_other_agent() {
    ...
    svc.remember(
        alice,
        "Recent event: referendum passed",
        MemoryType::Episodic,
        None,
    )
    .await
    ...
    let results = svc
        .recall(bob, "referendum passed", 10)
        .await
        .expect("recall");
...
async fn mi_14_old_memory_not_visible_to_other_agent() {
    ...
    svc.remember(
        alice,
        "Historical: Polkadot launched in 2020",
        MemoryType::Semantic,
        None,
    )
    .await
    ...
    let results = svc
        .recall(bob, "Polkadot launched", 10)
        .await
        .expect("recall");
...
async fn mi_15_boundary_timestamp_isolation() {
    ...
    let alice_results = svc
        .recall(alice, "boundary test memory", 10)
        .await
        .expect("recall");
    let bob_results = svc
        .recall(bob, "boundary test memory", 10)
        .await
        .expect("recall");
...
async fn mi_16_simultaneous_stores_isolated() {
    ...
    let alice_results = svc
        .recall(alice, "concurrent store", 10)
        .await
        .expect("recall");
    let bob_results = svc
        .recall(bob, "concurrent store", 10)
        .await
        .expect("recall");
    ...
    for r in &alice_results {
        assert!(
            ...
            "MI-16: alice query must not return bob's data"
        );
    }
    for r in &bob_results {
        assert!(
            r.content.contains("bob"),
            "MI-16: bob query must not return alice's data"
        );
    }
}
...
async fn mi_17_identical_content_different_agents() {
    ...
    svc.remember(alice, content, MemoryType::Semantic, None)
        .await
        .expect("alice store");
    svc.remember(bob, content, MemoryType::Semantic, None)
        .await
        .expect("bob store");

    let alice_results = svc
        .recall(alice, "existential deposit", 10)
        .await
        .expect("recall");
    let bob_results = svc
        .recall(bob, "existential deposit", 10)
        .await
        .expect("recall");
...
async fn mi_18_substring_search_isolated() {
    ...
    assert!(
        results.is_empty(),
        "MI-18: substring match must not cross agents"
    );
}
...
async fn mi_19_wildcard_recall_isolated() {
    ...
    svc.remember(
        alice,
        "Memory about governance proposals",
        MemoryType::Semantic,
        None,
    )
    .await
    ...
    assert!(
        results.is_empty(),
        "MI-19: broad recall must not cross agents"
    );
}
...
async fn mi_20_empty_query_isolation() {
    ...
    for r in &results {
        assert_ne!(
            r.agent_id, alice,
            "MI-20: empty query must not return other agent's data"
        );
    }
}
...
async fn mi_21_mixed_types_isolated() {
    ...
    assert!(
        results.is_empty(),
        "MI-21: all memory types isolated from other agent"
    );
}
...
async fn mi_22_many_agents_fully_isolated() {
    ...
    for (i, agent) in agents.iter().enumerate() {
        svc.remember(
            *agent,
            &format!("Secret data for agent {i}"),
            MemoryType::Semantic,
            None,
        )
        .await
