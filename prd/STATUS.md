...
| 19 Interactive/ACP | ACP server, durable terminal chat/TUI, shared runtime, headless interaction service, HTTP adapter, and command handlers implemented | TUI, chat, and ACP share conversation-scoped `/runs` and `/inspect`; chat, API, and TUI approval adapters share the service boundary but remain production-unbound; TUI adds bounded correlated multi-activity control; ACP maps its session ID exactly to the durable conversation UUID and projects effect-backed tool updates | Cross-surface restart identity, scoped/redacted run reads, durable approval retry, and controlled simultaneous TUI/approval execution pass; production authority, ACP approval binding, harness context, durable group/child-run orchestration, ACP session list/import, and manual Zed remain | Active P0/P1 |
...
- `crates/polkagent-runtime/src/interaction.rs` projects that exact persisted
  ...
  and TUI F6 actions through that service boundary. Production permission
  authority plus ACP binding remain unimplemented.
