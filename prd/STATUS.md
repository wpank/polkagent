...
| Effects/approvals/policy | Strict opt-in policy composition, SQLite V18 approval/checkpoint coordination, bounded APR-03 pause/recovery, APR-05 scoped service/HTTP operations, and the APR-06 terminal-chat adapter seam | Atomic pause, stable policy/tool/checkpoint digests, exact one-shot grant execution, no-I/O denial/expiry/cancel reduction, stable restart projection, idempotent decision retry, wrong-scope refusal, recovery wake, and chat dispatch are proved in injected/test-composed SQLite paths | `RuntimeFactory` does not bind a stable authenticated principal, so production HTTP/chat resolution and grant-bearing tools remain disabled. TUI/ACP bindings, the cross-surface crash matrix, coordinator-aware cancellation, cancellation during handler I/O, budgets, and effect drain remain. |
| Conversations/memory | `InteractionService` atomically persists transcript/run correlation/replay/context and exposes scoped durable approval operations; API and terminal chat have test-composable principal-bound adapters, and the API projects the runtime-owned SQLite memory store | Headless service, TUI, terminal chat, HTTP API, and ACP consume the durable lifecycle; typed memory operations and scoped approval retry survive service restart | One exact successful restarted interaction is cross-surface tested; approval projection and terminal/API dispatch are restart-tested, but production authority plus TUI/ACP actions are unbound; general memory is not assembled into prompt context. |
...
| 19 Interactive/ACP | ACP server, durable terminal chat/TUI, shared runtime, headless interaction service, HTTP adapter, and command handlers implemented | TUI, chat, and ACP share conversation-scoped `/runs` and `/inspect`; chat and API approval adapters are test-composable but production-unbound; TUI adds bounded correlated multi-activity control; ACP maps its session ID exactly to the durable conversation UUID and projects effect-backed tool updates | Cross-surface restart identity, scoped/redacted run reads, durable approval service/chat retry, and controlled simultaneous TUI execution pass; production authority, TUI/ACP approval bindings, harness context, durable group/child-run orchestration, ACP session list/import, and manual Zed remain | Active P0/P1 |
...
- `crates/polkagent-runtime/src/interaction.rs` projects that exact persisted
  ...
  through that service under explicit test composition. Production permission
  authority plus TUI/ACP bindings remain unimplemented.
...
- [`APPROVAL-PAUSE-RESUME-DESIGN.md`](APPROVAL-PAUSE-RESUME-DESIGN.md) records
  ...
  the remaining APR-06 TUI, APR-07 ACP, and APR-08 production/crash packets,
