...
and a shared interaction contract now exist
...
| Interactive terminal chat | Shared domain/service/command contracts exist; no adapter | Missing | Requires the durable `InteractionService` implementation, runtime factory, and terminal adapter. |
...
- `crates/polkagent-interaction` freezes surface-neutral interaction IDs,
  ...
  stream and service traits, and one typed 12-command MVP registry/parser. Its
  unit and property tests prove the bounded contract only: it has no migrations,
  durable service/projection implementation, command handlers, runtime binding,
  or terminal/TUI/API/ACP adapter yet.
