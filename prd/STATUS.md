...
| Monitoring TUI | Rich views plus a durable F9 Console and conversation-scoped F6 approval adapter, bounded async input/controller pump, single-flight background SQLite/chain projections, grapheme-safe multiline editor/history/paste, executable shared-command subset, guarded terminal lifecycle, durable session/agent selection, scoped run list/inspection, and correlated multi-activity control | Responsive and actionable for up to eight distinct agent/conversation turns, including while monitoring, read-only run commands, or approval requests execute, and restart-resumable. Explicit `polkagent tui` can opt into local-process approval authority; the no-subcommand TUI and omitted tuple remain fail-closed. | F6 lists at most 100 exact pending IDs for the selected Console conversation, shows bounded/redacted service detail and full confirmation IDs, and resolves only through `InteractionService`; stale switches and authority-unavailable composition fail closed. Cancel/shutdown coordinator-cancels a pending approval/run with zero tool I/O. Direct approval SQL is removed. Durable group plans/child runs, harness context, rich plans, shared/remote authentication, pagination, and cancellation during handler I/O remain. |
...
| Interactive terminal chat | `polkagent chat` uses the durable runtime interaction service and shared command handlers; the all-or-none authority tuple binds a local process principal and fixes the surface to `terminal-chat` | Usable single-agent, target/model-selectable line-mode session; explicit local authority enables scoped pending status plus `/approve` and `/deny`, while omission stays fail-closed | Interactive/non-TTY prompt, multiline input, contextual follow-up, transcript resume, persisted conversation-scoped `/agent` and `/model`, bounded `/runs` and `/inspect`, safe tool/approval status, lag replay, SIGINT cancellation, and coordinator-backed pending cancellation work. The tuple is not shared/remote authentication; provider/harness/autonomy changes, handler-I/O cancellation, harness follow-up, rich content, and groups remain unavailable. |
...
| Tools/skills | Registries and handlers plus real bounded grantless and injected approval-capable orchestrator paths with effect-backed projection | Grantless allowlisted tools execute in the normal composed loop; explicit local chat/TUI/ACP authority can expose the bounded approval-gated path, and APR-08 proves one real strict-policy governance call returns to the model exactly once | Default/API/shared-remote surfaces still withhold grant-bearing tools; raw arguments/output redaction, schema-wide validation, cancellation during I/O, broader recovery, and external-tool proof remain. |
| Effects/approvals/policy | Strict opt-in policy composition, SQLite V18 approval/checkpoint coordination, bounded APR-03 pause/recovery, APR-05 scoped service/HTTP operations, APR-06 terminal-chat/TUI seams, explicit local chat/TUI/ACP authority from `399db83`, and APR-08 fixture `20dbcb1` | Atomic pause, stable policy/tool/checkpoint digests, exact one-shot grant execution, no-I/O denial/expiry/cancel reduction, stable restart identities, idempotent decision retry, wrong-scope/principal refusal, one handler attempt/outcome, zero pending/orphan rows, and native ACP dispatch are proved | `RuntimeFactory` remains unbound by default and the production HTTP API lacks stable principal composition. Shared/remote authentication, the full crash-boundary matrix, possible-I/O reconciliation, cancellation during handler I/O, security/observability closure, budgets, and effect drain remain. |
| Conversations/memory | `InteractionService` atomically persists transcript/run correlation/replay/context and exposes scoped durable approval operations; API, terminal chat, TUI, and ACP consume that boundary, and the API projects the runtime-owned SQLite memory store | Headless service, TUI, terminal chat, HTTP API, and ACP consume the durable lifecycle; typed memory operations and scoped approval retry survive service/controller restart | APR-08 proves one exact policy-required interaction across TUI/chat and restart with identical conversation/approval/effect/run/tool-call IDs. Explicit local chat/TUI/ACP authority is available; default/API/shared-remote authority remains unbound and general memory is not assembled into prompt context. |
...
| Security | Grants, strict opt-in policy loading/composition, exact approval authority scope, injected orchestrator enforcement, tests, redaction, and signer abstractions exist | Bounded approval enforcement is locally usable by explicit chat/TUI/ACP process authority; the scoped HTTP contract is test-composed but production-unavailable | The local tuple is process-owner asserted, not authenticated shared/remote identity. Plaintext file secrets, shared-key API auth without per-key stable principal mapping, mock KMS/DID paths, production tenant/principal composition, and broader policy/grant coverage remain. |
...
| 13 UX | Durable terminal chat plus monitoring TUI with durable actionable Console and F6 approvals | Partial | Contextual follow-up/restart, bounded simultaneous activities, exact cancel, persisted selection, safe tool status, shared run commands, and service-routed approval queue/detail/actions are tested. Explicit local chat/TUI authority works; shared/remote/API authority, approval pagination, handler-I/O cancellation, harness context, broader commands, and group orchestration remain | Active P0/P1 + PRD-19 |
...
| 19 Interactive/ACP | ACP server, durable terminal chat/TUI, shared runtime, headless interaction service, HTTP adapter, and command handlers implemented | TUI, chat, and ACP share conversation-scoped `/runs` and `/inspect`; explicit local-process authority binds approval execution for chat/TUI/ACP, while default and API composition remain unbound; TUI adds bounded correlated multi-activity control; ACP maps its session ID exactly to the durable conversation UUID and projects effect-backed tool updates | APR-08 proves one strict-policy TUI/chat path with restart-stable identities, scoped/redacted projection, one attempt/outcome, and zero pending/orphans. Shared/remote/API authority, the broader crash matrix, handler-I/O cancellation, harness context, durable group/child-run orchestration, ACP session list/import, and manual Zed remain | Active P0/P1 |
...
- `crates/polkagent-runtime/src/interaction.rs` projects that exact persisted
  ...
  redaction contract exists. APR-05 projects durable approval identities and
  decisions through the coordinator, and APR-06 routes terminal-chat commands
  and TUI F6 actions through that service boundary. Commit `399db83` binds the
  same explicit local-process authority tuple to chat, TUI, and ACP; defaults
  and API composition remain fail-closed, and shared/remote authentication is
  still unimplemented.
...
- `crates/polkagent-interaction` freezes surface-neutral interaction IDs,
  ...
  harness, autonomy, max-turn, and budget remain explicitly unsupported;
  approve/deny require an explicitly bound authority and are unavailable in
  the normal production factory. For model executors, the service sends
...
- [`APPROVAL-PAUSE-RESUME-DESIGN.md`](APPROVAL-PAUSE-RESUME-DESIGN.md) records
  ...
  The narrow HTTP adapter is auth/read-only/OpenAPI tested, but production API
  composition intentionally leaves stable principal authority and grant-
  bearing readiness unbound. APR-07's local ACP packet is complete, and
  `399db83` extends the explicit local authority seam to chat and TUI. APR-08
  commit `20dbcb1` adds
  [`approval_cross_surface_e2e.rs`](../crates/polkagent-cli/tests/approval_cross_surface_e2e.rs):
  one real file-backed SQLite strict-policy governance tool is approved through
  TUI and checked through terminal chat's shared command executor across
  restart, preserving exact identities with one attempt/outcome, zero pending
  or orphan rows, and bounded/redacted projection. `4b4f11b` fixes the
  integrated `RuntimeFactory` future-size boundary. The broader crash matrix,
  manual Zed, shared/remote auth, security/observability, and cancellation
  during handler I/O remain open.
